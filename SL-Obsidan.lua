-- START: unmodified loading animation (as provided by user)
...
-- 工具函数
----------------------------------------
local function makeTween(name, speed, style, dir, loop, rev, delay)
    speed = speed or 0.17
    ...
    rev = rev or false
    delay = delay or 0
    return TweenInfo.new(speed, style, dir, loop, rev, delay)
end
...
local function drag(obj, latency)
    ...
    obj.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
            ...
            inp.Changed:Connect(function()
                if inp.UserInputState == Enum.UserInputState.End then toggled = false end
            end)
        end
    end)
    ...
    UIS.InputChanged:Connect(function(inp)
        if inp == dragInput and toggled then updateInput(inp) end
    end)
end
...
function library:Watermark(text)
    for _, v in ipairs(CoreGui:GetChildren()) do
        if v.Name == "watermark" then v:Destroy() end
    end
    text = text or "fps: " .. library.fps

    local wm = Instance.new("ScreenGui", CoreGui)
    wm.Name = "watermark"

    local edge = Instance.new("Frame", wm)
    edge.AnchorPoint = Vector2.new(0, 0)
    edge.Position = UDim2.new(0, 10, 0, 10) -- 左上角
    ...
    local bg = Instance.new("Frame", edge)
    ...
    g.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(34, 34, 34)),
-- END: unmodified loading animation

-- START: main UI library (modified to wait for LoadingAnimation)
...
-- // Loader integration (xsx v2 mobile) \\
-- This block will look for an existing loading animation ScreenGui named "LoadingAnimation" (searching CoreGui, PlayerGui, ReplicatedStorage, ReplicatedFirst).
-- If found, it will parent it to CoreGui and wait until it signals completion without modifying the animation.
-- Supported completion signals (in order of priority):
--  * BindableEvent named "Finished" (fires when done) -> waits .Event
--  * BoolValue or IntValue or NumberValue named "Finished" or "Loaded" turning true / >=1
--  * Attribute "Loaded" set to true
-- If none found, the UI will continue immediately (no-op) — place your exact animation ScreenGui named "LoadingAnimation" if you want blocking behavior.

local function WaitForLoadingAnimation(timeoutSeconds)
    ...
    local function findLoader()
        ...
        for _, place in ipairs(places) do
            if not place then continue end
            local g = place:FindFirstChild("LoadingAnimation") or place:FindFirstChild("Loader") or place:FindFirstChild("loadingAnimation")
    ...
    if not loader then
        -- No provided animation found; continue immediately.
        return true
    end

    -- Move the loader to CoreGui so it's visible and not interfered with.
    pcall(function() loader.Parent = CoreGui end)

    local done = false
    local finishedEvent

    -- Prefer BindableEvent "Finished"
    finishedEvent = loader:FindFirstChild("Finished")
    ...
    if not done then
        -- Watch for BoolValue/NumberValue named "Loaded"/"Finished"
        local function waitForValue(name)
            local v = loader:FindFirstChild(name)
            if v and (v:IsA("BoolValue") or v:IsA("NumberValue") or v:IsA("IntValue")) then
                if v.Value == true or (v:IsA("NumberValue") or v:IsA("IntValue") and v.Value >= 1) then
                    return true
                end
                local bind = v.Changed:Connect(function(new)
                    if new == true or (type(new) == "number" and new >= 1) then
                        bind:Disconnect()
                        done = true
                    end
                end)
                -- wait until done or timeout handled below
        ...
        if not waitForValue("Loaded") and not waitForValue("Finished") then
            -- check for attribute "Loaded"
            if loader:GetAttribute("Loaded") == true then
                done = true
            else
                -- fallback: wait until a child named "Finished" appears and becomes true
                local start = tick()
                while tick() - start < timeoutSeconds and not done do
                    if loader:GetAttribute("Loaded") == true then
                        done = true
                        break
                    end
                    local f = loader:FindFirstChild("Finished")
                    if f and (f:IsA("BoolValue") or f:IsA("NumberValue") or f:IsA("IntValue")) then
                        if f.Value == true or (f:IsA("NumberValue") and f.Value >= 1) then
                            done = true
                            break
                        end
                    end
    ...
    -- Optionally destroy or keep loader (we won't modify it).
    return done
end

-- Wait for animation to finish (if present) before allowing the rest of the UI to build.
-- If you prefer the UI to always wait, make sure your ScreenGui is named exactly "LoadingAnimation" and it signals completion
-- by one of the supported methods above (BindableEvent "Finished" recommended).
local _ok = WaitForLoadingAnimation(30)
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        local KeybindsToggle = { Normal = KeyPicker.Mode ~= "Toggle" }; do
            ...
            -- 绑定引用：方便当所属 Toggle（功能）切换时同步显示/隐藏快捷菜单条目
            KeybindsToggle.KeyPicker = KeyPicker
            KeybindsToggle.ParentObj = ParentObj
        ...
        function Toggle:SetValue(Value)
            ...
            for _, Addon in pairs(Toggle.Addons) do
                if Addon.Type == "KeyPicker" and Addon.SyncToggleState then
                    ...
                    -- 如果存在对应的 KeybindsToggle（快捷菜单条目），同步其可见性与显示状态
                    if Addon.KeybindsToggle then
                        pcall(function()
                            Addon.KeybindsToggle:SetVisibility(Toggle.Value)
                            Addon.KeybindsToggle:Display(Toggle.Value)
                        end)
                    end
                end
            end
...
function Library:CreateWindow(WindowInfo)
    ...
    if Library.IsMobile then
        local ToggleButton = Library:AddDraggableButton("打开/关闭", function()
            Library:Toggle()
        end)

        local LockButton = Library:AddDraggableButton("未锁定", function(self)
            Library.CantDragForced = not Library.CantDragForced
            self:SetText(Library.CantDragForced and "解锁" or "未锁定")
        end)
