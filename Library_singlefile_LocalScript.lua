...
-- ====== 以下为示例使用（放在同一文件，直接作为 LocalScript 使用） ======
-- 在 LocalScript 中运行此文件（放到 StarterPlayerScripts），如果库已定义为全局/局部变量 Library，会直接使用它。

local Library = Library or _LIB_LOADED
if not Library then
    -- 有些库使用 return Library；尝试从本脚本环境获取（如果是 Module 风格）
    if script and script:FindFirstChild("Library_module") then
        pcall(function() Library = require(script.Library_module) end)
    end
end

if not Library then
    warn("[示例整合脚本] 未能找到 Library 变量。请确认库代码在此文件顶部已正确声明 Library 并且没有语法错误。")
else
    ...
    local Tab = Window:AddTab("演示")
    local LeftGroup = Tab:AddLeftGroupbox("基础控件")
    LeftGroup:AddToggle("example_toggle", { Text = "示例开关", Default = false, Callback = function(v) print("开关", v) end })
    LeftGroup:AddSlider("example_slider", { Text = "示例滑块", Default = 50, Min = 0, Max = 100, Callback = function(v) print("滑块", v) end })
    LeftGroup:AddInput("example_input", { Text = "输入示例", Default = "", Placeholder = "请输入文字", Finished = true, Callback = function(v) print("输入:", v) end })

    local RightGroup = Tab:AddRightGroupbox("下拉/按钮")
    RightGroup:AddDropdown("example_dropdown", { Values = {"选项一","选项二","选项三"}, Default = "选项一", Multi = false, Text = "示例下拉", Callback = function(v) print("下拉选择", v) end })
    RightGroup:AddButton("示例按钮", function() print("示例按钮被点击") end)
    RightGroup:AddKeyPicker("example_keybind", { Text = "按键绑定", Default = Enum.KeyCode.RightShift, Callback = function(k) print("按键:", k) end })
