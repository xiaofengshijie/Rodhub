...
local Library = {
    ...
    Scheme = {
        ...
        MainColor = Color3.fromRGB(180, 220, 255),
        AccentColor = Color3.fromRGB(200, 230, 255),
        OutlineColor = Color3.fromRGB(120, 170, 220),
...
local Templates = {
    ...
    ColorPicker = {
        Default = Color3.fromRGB(200, 230, 255),
...
-- Preloader: 播放加载動畫（保留紫黑配色，不受主题色影响）
do
    local PreloaderGui = Instance.new("ScreenGui")
    PreloaderGui.Name = "ObsidianPreloader"
    PreloaderGui.ResetOnSpawn = false
    PreloaderGui.DisplayOrder = 1000
    PreloaderGui.Parent = gethui()

    local Frame = Instance.new("Frame")
    Frame.AnchorPoint = Vector2.new(0.5, 0.5)
    Frame.Position = UDim2.fromScale(0.5, 0.5)
    Frame.Size = UDim2.fromOffset(380, 140)
    Frame.BackgroundTransparency = 1
    Frame.Parent = PreloaderGui

    -- 背景条：紫到黑渐变（独立于 Library.Scheme）
    local Bg = Instance.new("Frame", Frame)
    Bg.AnchorPoint = Vector2.new(0.5, 0.5)
    Bg.Position = UDim2.fromScale(0.5, 0.5)
    Bg.Size = UDim2.fromScale(1, 1)
    Bg.BackgroundColor3 = Color3.fromRGB(20, 8, 35)
    Bg.BorderSizePixel = 0
    Bg.ClipsDescendants = true
    Bg.BackgroundTransparency = 0
    Bg.ZIndex = 1
    local bgGrad = Instance.new("UIGradient", Bg)
    bgGrad.Color = ColorSequence.new{ ColorSequenceKeypoint.new(0, Color3.fromRGB(60, 20, 120)), ColorSequenceKeypoint.new(1, Color3.fromRGB(8, 8, 10)) }
    ...
    Title.Text = "xsx v2 mobile edition\n加载中"
    Title.Font = Enum.Font.Code
    Title.TextColor3 = Color3.fromRGB(200, 180, 255) -- 浅紫色文字
    Title.TextStrokeTransparency = 0.7
    Title.TextWrapped = true
    Title.ZIndex = 2

    local DotLabel = Instance.new("TextLabel", Bg)
    DotLabel.Size = UDim2.new(1, -20, 0.4, -6)
    DotLabel.Position = UDim2.new(0, 10, 0.6, 6)
    DotLabel.BackgroundTransparency = 1
    DotLabel.TextScaled = true
    DotLabel.Text = ""
    DotLabel.Font = Enum.Font.Code
    DotLabel.TextColor3 = Color3.fromRGB(200, 180, 255)
    DotLabel.ZIndex = 2

    -- 简单点点动画：三个点循环，持续约 1.6 秒
    task.spawn(function()
        local cycles = 4
        for c = 1, cycles do
            for i = 1, 3 do
        ...
        -- 播放完成，启用主 UI 并移除 preloader
        pcall(function()
            if typeof(ScreenGui) == 'Instance' then
                ScreenGui.Enabled = true
            end
        end)
        PreloaderGui:Destroy()
    end)
end
...
function Library:CreateWindow(WindowInfo)
    ...
    if Library.IsMobile then
        ...
        local LockButton = Library:AddDraggableButton("锁定", function(self)
            ...
            self:SetText(Library.CantDragForced and "解锁UI" or "锁定")
        end)
