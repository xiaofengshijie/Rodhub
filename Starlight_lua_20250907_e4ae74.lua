...
function NotificationModule.Init()
    ...
    -- 创建通知容器 - 使用CoreGui并忽略安全区
    self.AchievementsHolder = Instance.new("ScreenGui")
    self.AchievementsHolder.Name = "NotificationSystem"
    self.AchievementsHolder.Parent = CoreGui
    self.AchievementsHolder.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    self.AchievementsHolder.IgnoreGuiInset = true -- 忽略屏幕安全区，通知贴顶显示
    
    -- 通知样式配置
    self.NotificationStyle = {
        Width = 320, -- 通知宽度（稍微加宽）
        Height = 100, -- 通知高度（增加高度）
        Position_Start = UDim2.new(1, 5, 0, 40), -- 初始位置（右侧外，顶部下方40像素）
        Position_End = UDim2.new(1, -320, 0, 40), -- 滑入后位置（右侧显示，顶部下方40像素）
        SlideTime = 0.35, -- 滑入/滑出时间
        BorderColor = Color3.fromRGB(255, 0, 0), -- 鲜红色边框
        Spacing = 110 -- 通知之间的间距
    }
...
-- 更新所有通知位置
function NotificationModule:UpdateNotificationPositions()
    for i, notification in ipairs(self.activeNotifications) do
        local targetY = 40 + (i - 1) * self.NotificationStyle.Spacing -- 使用配置的间距
        notification:TweenPosition(
            UDim2.new(1, -self.NotificationStyle.Width, 0, targetY),
...
-- 通知功能实现
function NotificationModule:Notify(unsafeOptions)
    local options = {
        Title = unsafeOptions.Title or "No Title",
        Description = unsafeOptions.Description or "No Text",
        Reason = unsafeOptions.Reason or "",
        NotificationType = unsafeOptions.NotificationType or "NOTIFICATION",
        Image = unsafeOptions.Image or "6023426923",
        Time = unsafeOptions.Time or 3,
        TweenDuration = unsafeOptions.TweenDuration or 0.8
    }
    
    -- 计算新通知的位置
    self.notificationCount = self.notificationCount + 1
    local notificationIndex = self.notificationCount
    local targetY = 40 + (#self.activeNotifications * self.NotificationStyle.Spacing) -- 使用配置的间距
    
    -- 1. 创建通知框架
    local Notification = Instance.new("Frame")
    Notification.Name = "Notification_" .. notificationIndex
    Notification.Parent = self.AchievementsHolder
    Notification.BackgroundColor3 = Color3.fromRGB(0, 0, 0) -- 黑色背景
    Notification.BackgroundTransparency = 0.8 -- 高度透明
    Notification.BorderSizePixel = 0
    Notification.Size = UDim2.new(0, self.NotificationStyle.Width, 0, self.NotificationStyle.Height)
    Notification.Position = UDim2.new(1, 5, 0, targetY) -- 初始位置
    Notification.ClipsDescendants = true -- 防止元素溢出
    Notification.ZIndex = 10 + notificationIndex -- 确保新通知显示在顶部
    ...
    stroke.Color = self.NotificationStyle.BorderColor -- 使用固定的鲜红色
    stroke.Thickness = 3 -- 加粗边框
    ...
    glow.ImageColor3 = self.NotificationStyle.BorderColor -- 使用固定的鲜红色
    glow.ImageTransparency = 0.3 -- 降低透明度使发光更明显
    ...
    TypeLabel.Text = options.NotificationType
    TypeLabel.Font = Enum.Font.GothamBold
    TypeLabel.TextSize = 18
    TypeLabel.TextColor3 = self.NotificationStyle.BorderColor -- 使用固定的鲜红色
    TypeLabel.Size = UDim2.new(0.3, 0, 0.15, 0)
    TypeLabel.Position = UDim2.new(0.02, 0, 0.05, 0)
    TypeLabel.BackgroundTransparency = 1
    TypeLabel.TextXAlignment = Enum.TextXAlignment.Left
    TypeLabel.Parent = Notification
    ...
    Icon.Size = UDim2.new(0, 40, 0, 40) -- 稍微增大图标
    Icon.Image = options.Image:match("rbxthumb://") or options.Image:match("rbxassetid://") and options.Image or "rbxassetid://" .. options.Image
    ...
    Details.Size = UDim2.new(0.75, 0, 0.8, 0) -- 调整大小以适应新高度
    Details.Position = UDim2.new(0.2, 0, 0.1, 0) -- 调整位置
    ...
    Description.TextColor3 = Color3.fromRGB(220, 220, 220) -- 亮灰色文字
    ...
    Reason.Size = UDim2.new(1, 0, 0.25, 0)
    Reason.Position = UDim2.new(0, 0, 0.75, 0)
    Reason.Text = options.Reason
    Reason.TextColor3 = Color3.fromRGB(180, 180, 180) -- 灰色文字
    ...
    if options.NotificationType == "WARNING" then
        sound.SoundId = "rbxassetid://9111179080"
    else
    ...
    task.spawn(function()
        ...
        Notification:TweenPosition(
            ...
            Enum.EasingDirection.Out,
            Enum.EasingStyle.Sine,
            self.NotificationStyle.SlideTime,
            true
        )
        wait(self.NotificationStyle.SlideTime) -- 等待滑入完成
        ...
        wait(options.Time) -- 等待显示时间
        
        -- 2. 滑出屏幕（回到顶部右侧外）
        Notification:TweenPosition(
            UDim2.new(1, 5, 0, targetY),
            Enum.EasingDirection.Out,
            Enum.EasingStyle.Sine,
            self.NotificationStyle.SlideTime,
            true
        )
        wait(self.NotificationStyle.SlideTime) -- 等待滑出完成
...
-- 初始化通知系统
local notificationSystem = NotificationModule.Init()
...
-- 主要功能内容
local LeftGroupBox = Tabs.Main:AddLeftGroupbox('移动功能')
...
function SpeedBypass:ToggleBypass(value)
    ...
    if value then
        ...
        Library:Notify("速度绕过", "速度绕过已启用，速度上限提升至32")
    else
        SpeedBoost.MaxSpeed = 22  -- 关闭速度绕过时，最大速度为22
        Library:Notify("速度绕过", "速度绕过已禁用，速度上限恢复至22")
    end
...
LeftGroupBox:AddToggle('SpeedBypassToggle', {
    Text = '速度绕过',
    Default = false,
...
LeftGroupBox:AddSlider('BypassDelaySlider', {
    Text = '绕过延迟',
    Default = 0.23,
    Min = 0.21,
    Max = 0.28,
...
-- 添加控制按钮
local ControlGroupBoxESP = Tabs.ESP:AddLeftGroupbox('控制')
ControlGroupBoxESP:AddButton('刷新ESP', function()
    CleanupESP()
    InitializeESP()
    Library:Notify("ESP", "ESP已刷新")
end)

ControlGroupBoxESP:AddButton('清除ESP', function()
    CleanupESP()
    Library:Notify("ESP", "ESP已清除")
end)
...
local function SetupForceJump()
    ...
    if AntiCheat.Settings.ForceJump then
        AntiCheat.Connections.ForceJump = game:GetService("RunService").Heartbeat:Connect(function()
            ...
            if LocalPlayer.Character and not LocalPlayer.Character:GetAttribute("CanJump") then
                LocalPlayer.Character:SetAttribute("CanJump", true)
            end
        end)
    else
...
ControlGroupBoxCheats:AddButton('刷新防怪功能', function()
    CleanupAntiFeatures()
    InitializeAntiFeatures()
    Library:Notify("防怪功能", "防怪功能已刷新")
end)

ControlGroupBoxCheats:AddButton('关闭所有防怪功能', function()
    CleanupAntiFeatures()
    Library:Notify("防怪功能", "所有防怪功能已关闭")
end)
...
local function setupEntityNotification()
    ...
    if EntityNotification.Enabled then
        ...
        Library:Notify("实体通知", "实体通知已启用")
    else
        Library:Notify("实体通知", "实体通知已禁用")
    end
end
...
LeftGroupBoxFloors:AddButton('刷新楼层信息', function()
    InitializeGameData()
    ...
    Library:Notify("楼层信息", "楼层信息已刷新")
end)

local RightGroupBoxFloors = Tabs.Floors:AddRightGroupbox('实体通知')
RightGroupBoxFloors:AddToggle('EntityNotificationToggle', {
    Text = '实体通知',
    Default = false,
...
-- 界面设置
local MenuGroup = Tabs['UI Settings']:AddLeftGroupbox('菜单管理')
...
MenuGroup:AddButton("重置设置", function()
    Library:Notify("界面设置", "设置已重置")
end)

local ThemeGroup = Tabs['UI Settings']:AddRightGroupbox('主题设置')
...
task.spawn(function()
    ...
    InitializeESP()
    InitializeAntiFeatures()
    Library:Notify("StarLight", "StarLight 加载成功!")
end)