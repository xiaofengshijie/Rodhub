...
local function FillInstance(Table: { [string]: any }, Instance: GuiObject)
    ...
    for k, v in pairs(Table) do
        ...
        elseif k ~= "Text" and (Library.Scheme[v] or typeof(v) == "function") then
            ...
            -- --- Begin: automatic slight-transparency for black/gray theme colors ---
            -- If the assigned theme key is one of the dark/gray keys, apply a small transparency
            -- so the UI becomes slightly transparent (mobile-friendly). We intentionally DO NOT
            -- change AccentColor (淡蓝 / accent) so that blue accents stay fully opaque.
            local __transparent_keys = {
                BackgroundColor = true,
                MainColor = true,
                OutlineColor = true,
                Dark = true,
            }
            if typeof(v) == "string" and __transparent_keys[v] then
                -- small transparency value (adjustable): 0.12 means 12% transparent
                local __obsidian_transparency = 0.12

                -- For background colors, set BackgroundTransparency
                if k == "BackgroundColor3" and Instance ~= nil then
                    -- only override if not explicitly set to fully transparent (1)
                    if Instance.BackgroundTransparency == nil or Instance.BackgroundTransparency == 0 then
                        Instance.BackgroundTransparency = __obsidian_transparency
                    end
                end

                -- For UIStroke objects (Outline), set its Transparency
                if Instance and typeof(Instance.IsA) == "function" and Instance:IsA("UIStroke") and k == "Color" then
                    Instance.Transparency = __obsidian_transparency
                end

                -- For image-like objects that set ImageColor3, avoid changing icons;
                -- only set ImageTransparency when an Image is used as a tiled/background (rare).
                if (k == "ImageColor3") and Instance and (typeof(Instance.IsA) == "function") then
                    if Instance:IsA("ImageLabel") or Instance:IsA("ImageButton") then
                        -- don't forcefully overwrite an already set transparency > 0; ensure slight transparency
                        local cur = Instance.ImageTransparency or 0
                        if cur == 0 then
                            Instance.ImageTransparency = __obsidian_transparency
                        end
                    end
                end
            end
            -- --- End: automatic slight-transparency ---
            continue
        end
