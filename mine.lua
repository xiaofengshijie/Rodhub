[file name]: Doors_complete_merged.txt
[file content begin]
...
local Executor = identifyexecutor()  or getexecutorname() or "未知"
local Library =  loadstring(game:HttpGet(repo..'Library.lua'))()

Library:Notify("正在加载 StarLight",5)
...
if not success then
Library:Notify("Require 不支持 某些功能将被禁用",3)
Disable1 = true
end
 
if not isnetworkowner then
Library:Notify("isnetworkowner 不支持 某些功能将被禁用",3)
Disable2 = true
end
...
if not success then
Prompt:Destroy()
Library:Notify("fireproximityprompt 不支持 方法将改变",4)
Disable3 = true
end
...
Disable4 = true
Library:Notify("replicatesignal 不支持 方法将改变",3)
end

if not hookmetamethod or not newcclosure then
Library:Notify("钩子不支持 某些功能将被禁用",3)
Disable5 = true
end
...
local ReachBox = Tabs.Main:AddLeftGroupbox('范围')

ESP = Tabs.Visuals:AddRightGroupbox('ESP')
SettingsESP = Tabs.Visuals:AddLeftGroupbox('设置')
ClosetTran = Tabs.Visuals:AddLeftGroupbox('透明化')
...
Anti:AddToggle('DeleteFigureFools',{
Text = "删除Figure (FE)",
Default = false,
Disabled = Disable2
})
...
Default = false,
Tooltip = (mode == "Fire") and "在范围内自动解锁挂锁" or "显示代码通知",
...
Library:Notify("自动启用防Figure听力",3)
end
...
AddESP(Door, (Door.Parent:FindFirstChild("Lock") and "已锁  " or "") .. "门 " .. Door.Parent:GetAttribute("RoomID"), DoorColor)
end
else
...
Library:Notify("已禁用自动柜子以便自动房间正常工作",4)
end
...
if Options.GMDropdown and Options.GMDropdown.Value == "自动化" then
...
SettingsBox:AddLabel("菜单绑定"):AddKeyPicker("MenuKeybind", { Default = "RightShift", NoUI = true, Text = "菜单绑定" })
...
local Contributors = Tabs.UISettings:AddRightGroupbox("制作人员")
Contributors:AddLabel("KardinCat - 创建者",true)
Contributors:AddLabel("notzanocoddz - 自动加载配置",true)
...
Library:Notify("已复制Discord链接到剪贴板",3)
