...
-- ========== Assistant appended runtime patch: enable built-in Crosshair & FPS, set colors, avoid duplicates ==========
pcall(function()
    local RunService = game:GetService("RunService")
    local CoreGui = game:GetService("CoreGui")

    -- Helper to safely get Library table (either local or global)
    local Lib = (Library ~= nil and Library) or (_G and _G.Library) or nil

    -- 1) Apply theme tweaks if possible (semi-transparent main color + light blue accent)
    if Lib and Lib.Scheme then
        pcall(function()
            Lib.Scheme.MainTransparency = 0.35
            Lib.Scheme.AccentColor = Color3.fromRGB(150,190,255)
            -- if MainColor exists and BackgroundTransparency hasn't been applied previously, leave it to library update routines
            if Lib.Scheme.MainColor and (not Lib.Scheme.MainTransparency) then
                Lib.Scheme.MainTransparency = 0.35
            end
        end)
    end

    -- 2) Enable built-in crosshair and FPS flags if the library exposes them
    pcall(function()
        if Lib and Lib.Flags then
            -- common flag names (try to set multiple possible names)
            local flagNames = {"ShowCrosshair", "Crosshair", "ShowFPS", "FPS", "ShowFps"}
            for _,name in ipairs(flagNames) do
                if Lib.Flags[name] ~= nil then
                    if name:lower():find("cross") then
                        Lib.Flags[name] = true
                    elseif name:lower():find("fps") then
                        Lib.Flags[name] = true
                    end
                end
            end
            -- also set explicit keys if missing
            if Lib.Flags.ShowCrosshair == nil then Lib.Flags.ShowCrosshair = true end
            if Lib.Flags.ShowFPS == nil then Lib.Flags.ShowFPS = true end
            if Lib.Flags.ShowFps == nil then Lib.Flags.ShowFps = true end
        end
    end)

    -- 3) Remove any previously appended "Library_Extras" GUI to avoid duplicate crosshair/FPS
    local extras = CoreGui:FindFirstChild("Library_Extras")
    if extras then
        extras:Destroy()
    end

    -- 4) Find any FPS label in CoreGui and cap displayed FPS at 60 (and initialize to 60)
    local function cap_and_bind_fps_label(label)
        if not label or not label.Parent or not label:IsA("TextLabel") then return end
        -- set initial text to 60
        pcall(function() label.Text = "FPS: 60" end)
        -- bind update to cap at 60
        local connName = "LibraryCapFPS_"..tostring(label)
        -- Avoid re-binding by checking for a BoolValue child marker
        if not label:FindFirstChild("_LibraryFPSBound") then
            local marker = Instance.new("BoolValue")
            marker.Name = "_LibraryFPSBound"
            marker.Parent = label
            RunService:BindToRenderStep(connName, Enum.RenderPriority.Last.Value, function(dt)
                if not label or not label.Parent then return end
                local fps = math.floor(1 / math.max(0.0001, dt) + 0.5)
    ...
    local function scan_for_fps_labels()
        for _,gui in pairs(CoreGui:GetChildren()) do
            for _,desc in pairs(gui:GetDescendants()) do
                if desc:IsA("TextLabel") then
                    local n = desc.Name:lower()
                    if n:find("fps") or n:find("fpslabel") or (desc.Text and tostring(desc.Text):lower():find("fps")) then
                        cap_and_bind_fps_label(desc)
                    end
                end
            end
        end
    ...
    scan_for_fps_labels()
    CoreGui.ChildAdded:Connect(function(_)
        -- small delay to let library create elements
        wait(0.05)
        scan_for_fps_labels()
    end)

    -- 5) Ensure we do not create a second crosshair: do not add any new crosshair here.
    --    If the library already has a crosshair, enabling its flag above should show it.
end)
-- ========== end runtime patch ==========
