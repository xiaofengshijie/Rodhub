...
-- 在UI加载前发送欢迎通知（带声音）
local function PlayNotificationSound(volume)
    volume = volume or 0.8  -- 增加默认音量
...
-- 发送欢迎通知（立即播放，不依赖UI设置）
PlayNotificationSound(0.8)  -- 增加音量
Library:Notify("欢迎使用 Lolhax！", 5)
...
    -- 先播放声音
    if Toggles and Toggles.NotificationSound and Toggles.NotificationSound.Value then
        local volume = Options and Options.NotificationSoundVolume and Options.NotificationSoundVolume.Value or 8
        PlayNotificationSound(volume / 10)  -- 将1-10的范围转换为0.1-1.0
    elseif not Toggles then
        -- 如果Toggles还未初始化，使用默认音量
        PlayNotificationSound(0.8)
    end
    
    -- 调用原始通知函数
    return originalNotify(self, message, duration)
end
...
-- 创建标签页
local Tabs = {
    General = Window:AddTab("通用", "wrench"),
    Exploit = Window:AddTab("利用", "file"),
    ESP = Window:AddTab("ESP", "eye"),
    Visuals = Window:AddTab("视觉", "theater"),
    Misc = Window:AddTab("其他", "moon"),
    Config = Window:AddTab("配置", "settings"),
}

-- 通用设置 - 自动化
local generalGroup = Tabs.General:AddLeftGroupbox("自动化", "rocket")
generalGroup:AddToggle("AutoInteract", {
    Text = "自动交互",
    Default = false,
})
...
        Text = "自动交互",
    })
generalGroup:AddDropdown("AutoInteractOptions", {
    Values = { "使用撬锁（门）", "使用撬锁（其他）", "忽略光源", "忽略糖果" },
    Default = 0,
    Multi = true,
    Text = "自动交互选项",
})
...
generalGroup:AddToggle("AutoCandy", {
    Text = "自动使用糖果",
    Default = false,
})
    :AddKeyPicker("AutoCandy", {
        Default = "H",
        SyncToggleState = false,
        Mode = "Toggle",
        Text = "自动使用糖果",
    })
generalGroup:AddToggle("AutoHide", {
    Text = "自动隐藏",
    Default = false,
})
generalGroup:AddToggle("AutoHideVisCheck", {
    Text = "预测可见检查",
    Default = false,
})
generalGroup:AddSlider("AutoHidePredTime", {
    Text = "预测时间",
    Default = 0.5,
    Min = 0.1,
    Max = 1.5,
...
generalGroup:AddSlider("AutoHidePredDistance", {
    Text = "距离倍数",
    Default = 1,
    Min = 0.8,
    Max = 1.5,
...
generalGroup:AddDivider()
generalGroup:AddToggle("MinecartInteract", {
    Text = "矿车交互spam",
    Default = false,
})
    :AddKeyPicker("MinecartInteract", {
        Default = "B",
        SyncToggleState = false,
        Mode = "Toggle",
        Text = "矿车交互spam",
    })
generalGroup:AddToggle("AnchorAutoSolve", {
    Text = "自动解决锚点",
    Default = false,
})
generalGroup:AddDivider()
generalGroup:AddToggle("AutoPadlock", {
    Text = "自动图书馆锁",
    Default = false,
})
generalGroup:AddSlider("AutoPadlockDistance", {
    Text = "自动锁距离",
    Default = 25,
    Min = 10,
    Max = 50,
...
-- 通用设置 - 通知
local notifyGroup = Tabs.General:AddRightGroupbox("通知", "bell")
notifyGroup:AddToggle("NotificationsEnabled", {
    Text = "启用通知",
    Default = true,
})
...
notifyGroup:AddSlider("NotificationSoundVolume", {
    Text = "音量",
    Default = 8,  -- 增加默认音量
    Min = 1,
    Max = 10,
...
notifyGroup:AddToggle("NotifyAnchorCode", {
    Text = "锚点代码",
    Default = false,
})
notifyGroup:AddToggle("NotifyPadlockCode", {
    Text = "图书馆锁码",
    Default = false,
})
...
notifyGroup:AddDropdown("EntityNotificationList", {
    Values = { "冲刺", "闪电战", "伏击", "凝视", "观察者", "停止", "尖叫", "蝙蝠群", "恐惧", "A-60", "A-120" },
    Default = 0,
    Multi = true,
    Text = "实体列表",
})
...
notifyGroup:AddSlider("NotificationOffset_X", {
    Text = "X偏移",
    Default = 0,
    Min = -1,
    Max = 1,
...
notifyGroup:AddSlider("NotificationOffset_Y", {
    Text = "Y偏移",
    Default = 0,
    Min = -1,
    Max = 1,
...
notifyGroup:AddSlider("NotificationSize", {
    Text = "大小倍数",
    Default = 1,
    Min = 0.8,
    Max = 3,
...
notifyGroup:AddButton("测试通知", function()
    Library:Notify("这是测试通知", 2.5)
end)

-- 利用设置 - 自我
local exploitSelf = Tabs.Exploit:AddLeftGroupbox("自我", "user")
exploitSelf:AddToggle("AlwaysJump", {
    Text = "始终启用跳跃",
    Default = false,
})
exploitSelf:AddDivider()
exploitSelf:AddToggle("AntiGloombat", {
    Text = "反蝙蝠蛋",
    Default = false,
})
exploitSelf:AddToggle("AntiGiggle", {
    Text = "反咯咯笑实体",
    Default = false,
})
exploitSelf:AddToggle("AntiSnare", {
    Text = "反陷阱",
    Default = false,
})
exploitSelf:AddToggle("AntiDupe", {
    Text = "反假门",
    Default = false,
})
exploitSelf:AddToggle("AntiEyes", {
    Text = "反凝视实体",
    Default = false,
})
exploitSelf:AddToggle("AntiLookman", {
    Text = "反观察者实体",
    Default = false,
})
exploitSelf:AddToggle("AntiChandelier", {
    Text = "反吊灯",
    Default = false,
})
exploitSelf:AddToggle("AntiSeekArms", {
    Text = "反搜寻臂",
    Default = false,
})

-- 利用设置 - 恶作剧
local exploitTroll = Tabs.Exploit:AddLeftGroupbox("恶作剧", "prank")
exploitTroll:AddToggle("SpamTools", {
    Text = "刷工具",
    Default = false,
})
    :AddKeyPicker("SpamTools", {
        Default = "G",
        SyncToggleState = false,
        Mode = "Toggle",
        Text = "刷工具",
    })
exploitTroll:AddInput("WhitelistPlayer", {
    Default = "",
    Numeric = false,
    Finished = true,
...
-- 利用设置 - 绕过
local exploitBypass = Tabs.Exploit:AddRightGroupbox("绕过", "bug")
exploitBypass:AddToggle("CrouchSpoof", {
    Text = "蹲伏伪装",
    Default = false,
})
exploitBypass:AddToggle("SpeedBypass", {
    Text = "速度绕过",
    Default = false,
})
exploitBypass:AddToggle("ACManipulate", {
    Text = "反作弊操作",
    Default = false,
})
    :AddKeyPicker("ACManipulate", {
        Default = "T",
        SyncToggleState = false,
        Mode = "Toggle",
        Text = "反作弊操作",
...
-- 利用设置 - 移除
local exploitRemovals = Tabs.Exploit:AddRightGroupbox("移除", "remove")
exploitRemovals:AddToggle("RemoveSeek", {
    Text = "移除搜寻追逐",
    Default = false,
})
exploitRemovals:AddToggle("NoScreech", {
    Text = "无尖叫",
    Default = false,
})
exploitRemovals:AddToggle("NoA90", {
    Text = "无A-90",
    Default = false,
})
exploitRemovals:AddToggle("NoHalt",
